  ...
  case moduleNameToPath modName of
    ...
    Left file ->
      do existsp <- liftIO $ doesFileExist file
         when (not existsp) $ debug $ NoSuchFile $ show file
         stx <- liftIO (readModule file) >>=
                \case
                  Left err -> debug $ ReaderError err
...
getEValue b = do
  ...
  case S.lookup b env of
    Just v -> return v
    Nothing -> debug (InternalError ("No such binding: " ++ show b))
...
checkUnambiguous best candidates blame =
  do p <- currentPhase
     ...
     if length (filter (== bestSize) candidateSizes) > 1
       then debug (Ambiguous p blame candidates)
       else return ()
...
resolve stx@(Stx scs srcLoc x) = do
  ...
  case bs of
    Seq.Empty ->
      do
        debug (Unknown (Stx scs srcLoc x))
    candidates ->
      let
...
primImportModule dest outScopesDest importStx = do
  ...
  where
    ...
    importSpec stx@(Syntax (Stx _ _ (List elts)))
      ...
      | otherwise = debug $ NotImportSpec stx
  ...
  where
              ...
              case args of
                (Syntax (Stx _ _ (Integer i)) : more) -> do
                  ...
                  if i >= 0
                    ...
                    else debug $ NotExportSpec blame
                _ -> debug $ NotExportSpec blame
            _ -> debug $ NotExportSpec blame
      | Just xs <- traverse getIdent elts = return (ExportIdents xs)
      | otherwise = debug $ NotExportSpec blame
    ...
    getRenames _blame (syntaxE -> List rens) =
      for rens $ \renStx -> do
        ...
        pure (x', y')
    getRenames blame _ = debug $ NotExportSpec blame
      ...
      case newDeps of
        ...
        [] ->
          ...
          \case
            ...
            Just macroImpl -> do
              p <- currentPhase
              macroImplVal <- inEarlierPhase $ evalInCurrentPhase macroImpl
      ...
      if ready
        then do
          ...
          case view (expanderExpressionTypes . at edest) st of
            Nothing -> debug $ InternalError "Type not found during generalization"
            Just _ -> do
      ...
      if not ready
        then stillStuck tid task
          ...
          let getVarInfo ptr =
                ...
                \case
                  Nothing ->
                    debug $ InternalError "Pattern info not added"
                  Just (Right found) ->
                    pure [found]
                  Just (Left ptrs) ->
    ...
    AwaitingTypePattern patPtr ty dest stx -> do
      ...
      if not ready
        then stillStuck tid task
          ...
          case varInfo of
            Nothing -> debug $ InternalError "Type pattern info not added"
            Just vars -> do
              p <- currentPhase
              let rhs' = foldr (addScope p) stx
                           [ sc'
...
expandOneForm prob stx
          ...
          case prob of
            ...
            PatternDest patTy dest -> do
              ...
              if length subPats /= length argTypes
                then debug $ WrongArgCount stx ctor (length argTypes) (length subPats)
                else do
  ...
  | otherwise =
    case prob of
      ModuleDest {} ->
        debug $ InternalError "All modules should be identifier-headed"
      DeclTreeDest {} ->
        debug $ InternalError "All declarations should be identifier-headed"
      TypeDest {} ->
        debug $ NotValidType stx
      ExprDest t dest ->
        case syntaxE stx of
          List xs -> expandOneExpression t dest (addApp stx xs)
      ...
      PatternDest {} ->
        debug $ InternalError "All patterns should be identifier-headed"
      TypePatternDest {} ->
        debug $ InternalError "All type patterns should be identifier-headed"
  ...
  \case
    ...
    MacroActionIdentEq how v1 v2 -> do
      ...
      where
        ...
        getIdent _other = debug $ InternalError $ "Not a syntax object in " ++ opName
